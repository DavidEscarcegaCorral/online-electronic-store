...
public class DatabaseInitializer {
    ...
    public static boolean ejecutarInicializacion() {
        try {
            ...
            return initializer.inicializarBaseDatos();
        } catch (Exception e) {
            LoggerFactory.getLogger(DatabaseInitializer.class).error("ERROR al inicializar la base de datos:", e);
            return false;
        }
    }
    ...
    private void limpiarBaseDatos() {
        ...
        for (String coleccion : colecciones) {
            try {
                ...
                if (eliminados > 0) {
                    logger.info("Colección '{}': {} documentos eliminados", coleccion, eliminados);
                }
            } catch (Exception e) {
                logger.info("Colección '{}' no existía, se creará nueva", coleccion);
            }
        }
    }
    ...
    private void crearColecciones() {
        ...
        for (String nombreColeccion : colecciones) {
            try {
                ...
                logger.debug("Colección '{}' creada", nombreColeccion);
            } catch (Exception e) {
                logger.debug("Colección '{}' ya existe", nombreColeccion);
            }
        }
    }
    ...
    private void crearIndices() {
        try {
            ...
            carritosCol.createIndex(new Document("clienteId", 1),
                    new com.mongodb.client.model.IndexOptions().unique(true));
            logger.debug("Índice único creado en carritos.clienteId");
        } catch (Exception e) {
            logger.debug("Índice en carritos.clienteId ya existe o error: {}", e.getMessage());
        }

        try {
            MongoCollection<Document> usuariosCol = database.getCollection("usuarios");
            usuariosCol.createIndex(new Document("email", 1),
                    new com.mongodb.client.model.IndexOptions().unique(true));
            logger.debug("Índice único creado en usuarios.email");
        } catch (Exception e) {
            logger.debug("Índice en usuarios.email ya existe o error: {}", e.getMessage());
        }

        try {
            MongoCollection<Document> configuracionesCol = database.getCollection("configuraciones");
            configuracionesCol.createIndex(new Document("usuarioId", 1));
            logger.debug("Índice creado en configuraciones.usuarioId");
        } catch (Exception e) {
            logger.debug("Índice en configuraciones.usuarioId ya existe o error: {}", e.getMessage());
        }
    }
    ...
    private void insertarUsuarioDefault() {
        ...
        logger.info("Usuario creado: {}", usuario.getString("nombre"));
        logger.info("ID: {}", usuarioDefaultId);
    }
    ...
    private void insertarCarritoDefault() {
        ...
        logger.info("Carrito creado para el usuario");
        logger.info("ID: {}", carritoDefaultId);
    }
    ...
    private void insertarPedidosEjemplo() {
        ...
        if (!pedidosEjemplo.isEmpty()) {
            ...
            logger.info("{} pedidos de ejemplo insertados", pedidosEjemplo.size());
        }
    }
    ...
    private void mostrarInserts() {
        ...
        logger.info("Carritos:         {}", database.getCollection("carritos").countDocuments());
        logger.info("Productos:        {}", database.getCollection("productos").countDocuments());
        logger.info("Configuraciones:  {}", database.getCollection("configuraciones").countDocuments());
        logger.info("Pedidos:          {}", database.getCollection("pedidos").countDocuments());
    }
    ...
    private Document crearProducto(String nombre, String categoria, String marca,
        ...
        doc.append("imagenUrl", generarUrlImagen(nombre, categoria));
        doc.append("especificaciones", new Document(especificaciones));
        doc.append("fechaCreacion", new Date());
        return doc;
    }

    private String generarUrlImagen(String nombreProducto, String categoria) {
